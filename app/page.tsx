...
import { leadOperations, type Lead, isSupabaseConfigured, supabase } from "@/lib/supabase-operations"
...
const loadLeads = async () => {
  ...
  try {
    ...
    // Determinar status da conexão (considera UUID e faz um ping se necessário)
    if (isSupabaseConfigured) {
      const hasRemoteLeads = loadedLeads.some(
        (lead) => typeof lead.id === "string" && !String(lead.id).startsWith("local_"),
      )
      ...
      if (hasRemoteLeads) {
        ...
        try {
          if (supabase) {
            const { error } = await supabase.from("lead").select("id", { count: "exact", head: true })
            setSupabaseStatus(error ? "local" : "connected")
          } else {
